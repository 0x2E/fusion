...
import { feedAPI, type CreateFeedRequest } from "@/lib/api";
...
export function AddFeedDialog() {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      toast.error("Failed to validate feed URL");
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    const selectedGroupId = groupId ? parseInt(groupId, 10) : groups[0]?.id ?? 1;
    ...
    <Dialog open={isAddFeedOpen} onOpenChange={setAddFeedOpen}>
      ...
      >
        ...
        <DialogHeader className="flex flex-row items-center justify-between border-b px-5 py-4">
          <DialogTitle className="text-base font-semibold">Add Feed</DialogTitle>
          <Button
            variant="ghost"
            size="icon"
            className="h-7 w-7"
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              ...
              >
                <Radar className={cn("h-[18px] w-[18px]", isValidating && "animate-pulse")} />
              </Button>
            </div>
